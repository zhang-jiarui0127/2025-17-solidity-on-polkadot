...
async function main() {
  const walletClient = createWalletClient({ chain: local, transport: http(), account })
  ...
  publicClient.watchEvent({
    ...
    onLogs: (logs) => {
      for (const log of logs) {
        const n = (log as any).args?.newNumber
        console.log("Event: NumberChanged =", n?.toString())
      }
    },
  })

  // 觸發事件
  await contract.write.set([456])
}
